...
function Encripta(chavePlano,textoPlano:string;lengthchave:integer):string;
var chaveNumero,TextoPlanoNumero,auxChave,auxTexto,auxencripta:string;
    soma,tam,cont,error,auxChaveNumero,auxTextoNumero,auxmodulo,posicao,modtamanhochave:integer;
    letra:char;
begin  // Recebe a chave do tipo texto, o texto plano e o tamanho da chave
     ...
     repeat  // loop que lê caractere por caractere do texto plano e insere em uma outra string
            // o respectivo caractere já encriptado
           inc(posicao);
           case TextoPlanoNumero[cont] of // verifica qual o tipo do caractere na posição cont
                chr(0)..chr(47) :insert(TextoPlanoNumero[cont],auxencripta,posicao);
 	        chr(48)..chr(57) :begin    // Compara de acordo com a tabela ASCII os números de 0 a 9  Se for número este número será convertido em letra
                                       ...
                                       soma:=modtamanhochave+auxTextoNumero;
                                       auxmodulo:=Modulo(soma,26);
                                       letra:=ConverteNumero2Ascii(auxmodulo);// Recebe o número encontrado em letra
                                       insert(letra,auxencripta,posicao); // Insere a letra na variável auxencripta na posição que está sendo lida
                ...
                chr(58)..chr(64) :insert(TextoPlanoNumero[cont],auxencripta,posicao);
                chr(91)..chr(97) :insert(TextoPlanoNumero[cont],auxencripta,posicao);
                chr(123)..chr(255) :insert(TextoPlanoNumero[cont],auxencripta,posicao);

           end;
	   inc(cont);
     until(posicao=tam);
     Encripta:=auxencripta;
end;

 //Função que imprime o texto criptografado em um arquivo de saída
procedure ImprimeArquivo(criptografado:string);
var f2: text;
begin
     assign(f2,ParamStr(3));
     rewrite(f2);
     write(f2,criptografado);
     close(f2);
end;

var chave,texto,chavePlano,criptografado: string;
...
begin
    ...
    criptografado:=Encripta(chavePlano,texto,tamanhochave);
    ImprimeArquivo(criptografado);
    textcolor(10);
    writeln('Criptografado! Arquivo de saída criado com sucesso!');
    textcolor(7);
end.